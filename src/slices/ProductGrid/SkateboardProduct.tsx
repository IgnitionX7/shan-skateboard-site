...
const VERTICAL_LINE_CLASSES =
  "absolute top-0 stroke-2 h-full w-1 text-stone-300 transition-colors group-hover:text-stone-400";
...
export async function SkateboardProduct({ id }: Props) {
  ...
  return (
    <div className="group relative mx-auto w-full max-w-72 px-8 pt-4">
      ...
      <div className="flex items-center justify-between text-sm sm:text-base md:text-xl lg:text-2xl">
        <span>{price}</span>
        <span className="inline-flex items-center gap-1">
          <FaStar className="text-yellow-400" /> 35
        </span>
      </div>
      ...
      <HorizontalLine className={HORIZONTAL_LINE_CLASSES} />
      <h3 className="my-2 text-center font-chicle leading-tight text-lg md:text-xl lg:text-2xl">
        {product.data.name}
      </h3>
